...
class ImageDataGenerator(object):
  ...
  def flow_from_directory(self,
                          ...
                          save_format='jpeg',
                          follow_links=False,
                          sub_directory_list=None,
                          sub_directory_y=None,
                          input_format=None):

    if class_mode == 'image': 
      if sub_directory_list and sub_directory_y:
        return DirectoryIterator_imageY(
                  directory,
                  self,
                  sub_directory_list,
                  sub_directory_y,
        ...
        raise ValueError("sub_directory_list and sub_directory_y required when using DirectoryIterator_imageY")
...
class DirectoryIterator_imageY(Iterator):
  """Iterator capable of reading images from a directory on disk.

  This iterator is used to read images as both the input and output.
    If you are transforming images into another image this is the correct
    DirectoryIterator to call from a flow_from_directory function.
  ...
  Arguments:
      directory: Path to the directory to read images from.
          ...
          or alternatively you could specify class subdirectories
          via the `classes` argument.
      sub_directory_list: The list of where the files are stored.
      image_data_generator: Instance of `ImageDataGenerator`
          to use for random transformations and normalization.
      target_size: tuple of integers, dimensions to resize input images to.
      color_mode: One of `"rgb"`, `"grayscale"`. Color mode to read images.
      class_mode: Mode for yielding the targets: No class mode, this is only 
        for dealing with image to image transformation.
      batch_size: Integer, size of a batch.
      shuffle: Boolean, whether to shuffle the data between epochs.
      seed: Random seed for data shuffling.
      data_format: String, one of `channels_first`, `channels_last`.
  ...
  def __init__(self,
    ...
    if isinstance(sub_directory_list, list):
      ...
      raise ValueError('sub_directory_list may only be passed a list' + 
        'object %s is of type %s' 
        %(sub_directory_list, type(sub_directory_list)))
    ...
    if input_format:
      ...
      self.input_format = {'png', 'jpg', 'jpeg', 'bmp'}
    ...
    for sub_dir_name in sub_directory_list:
      ...
      if self.samples == dir_samples or self.samples == 0:
        self.samples = dir_samples
      else:
        raise ValueError("All sub directories and Y subdriectories must ",
              "have the same number of images in it.")

    y_path = os.path.join(directory, sub_dir_name)
    self.y_filenames, y_samples = self._get_sub_dir_list(y_path)

    if self.samples != y_samples:
      raise ValueError("All sub directories and Y subdriectories must ",
            "have the same number of images in it.")

    super(DirectoryIterator_imageY, self).__init__(self.samples, batch_size, shuffle,
                                            seed)
  
  #We only want to have to do this check once per epoch
  def _get_sub_dir_list(self, full_subdir_path):
    file_list = []
    sub_dir_samples = 0
    if self.check_dict:
      for file_name in os.listdir(full_subdir_path):
        ...
        for extension in white_list_formats:
          if file_name.lower().endswith('.' + extension):
            is_valid = True
            sub_dir_samples = sub_dir_samples + 1
            break
        if is_valid:
          file_list.append(os.path.join(full_subdir_path, file_name))

    else:
      for file_name in os.listdir(full_subdir_path):
        if file_name.lower().endswith('.' + self.input_format):
          file_list.append(os.path.join(full_subdir_path, file_name))
          sub_dir_samples = sub_dir_samples + 1

    return file_list, sub_dir_samples
  ...
  def next(self):
    ...
    batches = []
    for batch_idx in self.sub_directory_list:
      cur_batch = np.zeros((current_batch_size,) + self.image_shape, dtype=K.floatx())
      batches.append(cur_batch)
      grayscale = self.color_mode == 'grayscale'

    n_dir = len(batches)
    #For this generator we have a list of index_arrays that are the same size
    #When we build a batch we draw from the same spot in each subdirectory
    y_batch = np.zeros((current_batch_size,) + self.image_shape, dtype=K.floatx())
    # print(index_array)
    # print(self.filenames)
    for i in range(0, len(index_array)):
      for dir_idx in range(0, n_dir):
        xname = self.filenames[dir_idx][i]
        x = load_img(
            xname,
            grayscale=grayscale,
            target_size=self.target_size)
